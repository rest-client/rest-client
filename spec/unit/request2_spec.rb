...
describe RestClient2::Request, :include_helpers do
  ...
  context 'params for GET requests' do
    it "manage params for get requests" do
      ...
      expect(RestClient2::Request.execute(:url => 'http://some/resource', :method => :get, :headers => {:foo => :bar, :params => {:a => :b, 'c' => 'd'}}).body).to eq 'foo'

      stub_request(:get, 'http://some/resource').with(:headers => {'Accept'=>'*/*', 'Foo'=>'bar'}).to_return(:body => 'foo', :status => 200)
      expect(RestClient2::Request.execute(:url => 'http://some/resource', :method => :get, :headers => {:foo => :bar, :params => :a}).body).to eq 'foo'
    end

    it 'adds GET params when params are present in URL' do
      stub_request(:get, 'http://some/resource?a=b&c=d').with(:headers => {'Accept'=>'*/*', 'Foo'=>'bar'}).to_return(:body => 'foo', :status => 200)
      expect(RestClient2::Request.execute(:url => 'http://some/resource?a=b', :method => :get, :headers => {:foo => :bar, :params => {:c => 'd'}}).body).to eq 'foo'
    end
    ...
    it 'encodes nested GET params' do
      ...
      expect(RestClient2::Request.execute(url: 'http://some/resource', method: :get, headers: {
        params: {
          a: {
            foo: [1,2],
            bar: nil,
  ...
  it "can use a block to process response" do
    ...
    stub_request(:get, 'http://some/resource?a=b&c=d').with(:headers => {'Accept'=>'*/*', 'Foo'=>'bar'}).to_return(:body => 'foo', :status => 200)
    RestClient2::Request.execute(:url => 'http://some/resource', :method => :get, :headers => {:foo => :bar, :params => {:a => :b, 'c' => 'd'}}, :block_response => block)
  ...
  it 'closes payload if not nil' do
    ...
    RestClient2::Request.execute(:url => 'http://some/resource', :method => :post, :payload => {:file => test_file})

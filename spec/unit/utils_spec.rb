...
describe RestClient2::Utils do
  describe '.get_encoding_from_headers' do
    it 'assumes no encoding by default for text' do
      headers = {:content_type => 'text/plain'}
      expect(RestClient2::Utils.get_encoding_from_headers(headers)).
        to eq nil
    end

    it 'returns nil on failures' do
      expect(RestClient2::Utils.get_encoding_from_headers(
  ...
  describe '.cgi_parse_header' do
    it 'parses headers', :unless => RUBY_VERSION.start_with?('2.0') do
      expect(RestClient2::Utils.cgi_parse_header('text/plain')).
        to eq ['text/plain', {}]

      expect(RestClient2::Utils.cgi_parse_header('text/vnd.just.made.this.up')).
        to eq ['text/vnd.just.made.this.up', {}]

      expect(RestClient2::Utils.cgi_parse_header('text/plain;charset=us-ascii')).
        to eq ['text/plain', {'charset' => 'us-ascii'}]

      expect(RestClient2::Utils.cgi_parse_header('text/plain ; charset="us-ascii"')).
        to eq ['text/plain', {'charset' => 'us-ascii'}]

      expect(RestClient2::Utils.cgi_parse_header(
  ...
  describe '.encode_query_string' do
    it 'handles simple hashes' do
      {
        ...
        expect(RestClient2::Utils.encode_query_string(input)).to eq expected
      end
    end
    ...
    it 'handles multiple fields with the same name using ParamsArray' do
      {
        RestClient2::ParamsArray.new([[:foo, 1], [:foo, 2], [:foo, 3]]) => 'foo=1&foo=2&foo=3',
      }.each_pair do |input, expected|
        expect(RestClient2::Utils.encode_query_string(input)).to eq expected
      end
    end
    ...
    it 'handles nested ParamsArrays' do
      {
        {foo: RestClient2::ParamsArray.new([[:a, 1], [:a, 2]])} => 'foo[a]=1&foo[a]=2',
        RestClient2::ParamsArray.new([[:foo, {a: 1}], [:foo, {a: 2}]]) => 'foo[a]=1&foo[a]=2',
      }.each_pair do |input, expected|
        expect(RestClient2::Utils.encode_query_string(input)).to eq expected
      end
    end

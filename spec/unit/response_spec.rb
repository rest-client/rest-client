...
describe RestClient2::Response, :include_helpers do
  before do
  ...
  it "accepts nil strings and sets it to empty for the case of HEAD" do
    # TODO
    expect(RestClient2::Response.create(nil, @net_http_res, @request).to_s).to eq ""
  end
  ...
  describe "cookie processing" do
    it "should correctly deal with one Set-Cookie header with one cookie inside" do
      ...
      response = RestClient2::Response.create('abc', net_http_res, @request)
  ...
  describe "exceptions processing" do
    it "should return itself for normal codes" do
      (200..206).each do |code|
        ...
        resp = RestClient2::Response.create('abc', net_http_res, @request)
        resp.return!
      end
    end

    it "should throw an exception for other codes" do
      RestClient2::Exceptions::EXCEPTIONS_MAP.each_pair do |code, exc|
  ...
  describe "redirection" do
    ...
    it "follows a redirection when the request is a get" do
      ...
      expect(RestClient2::Request.execute(:url => 'http://some/resource', :method => :get).body).to eq 'Foo'
    end

    it "keeps redirection history" do
      stub_request(:get, 'http://some/resource').to_return(:body => '', :status => 301, :headers => {'Location' => 'http://new/resource'})
      stub_request(:get, 'http://new/resource').to_return(:body => 'Foo')
      r = RestClient2::Request.execute(url: 'http://some/resource', method: :get)
      expect(r.body).to eq 'Foo'
      expect(r.history.length).to eq 1
      expect(r.history.fetch(0)).to be_a(RestClient2::Response)
    ...
    it "follows a redirection and keep the parameters" do
      ...
      expect(RestClient2::Request.execute(:url => 'http://some/resource', :method => :get, :user => 'foo', :password => 'bar', :headers => {:accept => :json}).body).to eq 'Foo'
    end
    ...
    it "follows a redirection and keep the cookies" do
      ...
      expect(RestClient2::Request.execute(:url => 'http://some/resource', :method => :get).body).to eq 'Qux'
    end
    ...
    it 'respects cookie domains on redirect' do
      ...
      expect(RestClient2::Request.execute(:url => 'http://some.example.com/', :method => :get, cookies: [HTTP::Cookie.new('passedthrough', '1', domain: 'new.example.com', path: '/')]).body).to eq 'Qux'
    end
    ...
    it "doesn't follow a 301 when the request is a post" do
      ...
      expect {
        response.return!
      }.to raise_error(RestClient2::MovedPermanently)
    end
    ...
    it "doesn't follow a 302 when the request is a post" do
      ...
      expect {
        response.return!
      }.to raise_error(RestClient2::Found)
    end
    ...
    it "doesn't follow a 307 when the request is a post" do
      ...
      expect {
        response.return!
      }.to raise_error(RestClient2::TemporaryRedirect)
    end
    ...
    it "follows a redirection when the request is a post and result is a 303" do
      ...
      stub_request(:get, 'http://new/resource').to_return(:body => 'Foo')
      expect(RestClient2::Request.execute(:url => 'http://some/resource', :method => :put).body).to eq 'Foo'
    end
    ...
    it "follows a redirection when the request is a head" do
      ...
      expect(RestClient2::Request.execute(:url => 'http://some/resource', :method => :head).body).to eq 'Foo'
    end
    ...
    it "follows no more than 10 redirections before raising error" do
      ...
      expect {
        RestClient2::Request.execute(url: 'http://some/redirect-1', method: :get)
      }.to raise_error(RestClient2::MovedPermanently) { |ex|
        ex.response.history.each {|r| expect(r).to be_a(RestClient2::Response) }
    ...
    it "follows no more than max_redirects redirections, if specified" do
      ...
      stub_request(:get, 'http://some/redirect-2').to_return(:body => '', :status => 301, :headers => {'Location' => 'http://some/redirect-2'})
      expect {
        RestClient2::Request.execute(url: 'http://some/redirect-1', method: :get, max_redirects: 5)
      }.to raise_error(RestClient2::MovedPermanently) { |ex|
    ...
    it "allows for manual following of redirects" do
      ...
      begin
        RestClient2::Request.execute(url: 'http://some/redirect-1', method: :get, max_redirects: 0)
      rescue RestClient2::MovedPermanently => err
  ...
  describe "logging" do
    it "uses the request's logger" do
      ...
      request = RestClient2::Request.new(url: 'http://some/resource', method: :get, log: logger)
